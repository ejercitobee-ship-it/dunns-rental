...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const row = await env.DB.prepare('SELECT id, name, photo_drive_id FROM properties WHERE id = ?')
      .bind(id)
      .first<{ id: string; name: string; photo_drive_id: string | null }>();
    ...
    await env.DB.batch([
      env.DB.prepare('UPDATE properties SET photo_drive_id = ?, updated_at = unixepoch() WHERE id = ?').bind(newId, id),
      logActivityStmt(env.DB, auth, {
        module: 'properties',
        action: 'Uploaded a property photo',
        targetType: 'properties',
        targetId: id,
        description: row.name,
      }),
    ]);
    return jsonOk({ success: true, data: { image: `/api/photo/${newId}` } });
  } catch (err) {
...
export const onRequestDelete: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    await env.DB.batch([
      env.DB.prepare('UPDATE properties SET photo_drive_id = NULL, updated_at = unixepoch() WHERE id = ?').bind(id),
      logActivityStmt(env.DB, auth, {
        module: 'properties',
        action: 'Removed a property photo',
        targetType: 'properties',
        targetId: id,
        description: row.name,
      }),
