...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO calendar_events (id, property_id, unit_id, title, description, category, event_date, event_time, priority, is_recurring, recurrence_rule, notes, user_id, reminder_hours)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      id,
      primaryPropertyId,
