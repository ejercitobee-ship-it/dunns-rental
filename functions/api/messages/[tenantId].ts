...
import { serializeMessage, notifyTenantOfReply, tenantContact, readMessageInput, MAX_ATTACHMENT_BYTES } from '../../lib/messages';
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Office attachments to a tenant go into that tenant's Drive folder too.
    ...
    await env.DB.prepare(
      `INSERT INTO messages (id, tenant_id, sender_role, sender_user_id, body, attachment_drive_id, attachment_name, attachment_type, read_by_office, read_by_tenant)
       VALUES (?, ?, 'office', ?, ?, ?, ?, ?, 1, 0)`
    ).bind(id, tenantId, auth.id, body, driveId, file?.name ?? null, file?.type ?? null).run();

    context.waitUntil(
      notifyTenantOfReply(env, tenantId, body || '(sent an attachment)').catch((e) => console.error('notifyTenantOfReply failed', e))
    );
