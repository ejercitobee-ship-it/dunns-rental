...
import { serializeMessage, notifyOfficeOfMessage } from '../../lib/messages';
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO messages (id, tenant_id, sender_role, sender_user_id, body, read_by_tenant, read_by_office)
       VALUES (?, ?, 'tenant', ?, ?, 1, 0)`
    ).bind(id, tenantId, auth.id, body).run();

    context.waitUntil(
      notifyOfficeOfMessage(env, tenantId, body).catch((e) => console.error('notifyOfficeOfMessage failed', e))
    );
